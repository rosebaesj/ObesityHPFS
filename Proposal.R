...
z_mlvs_exposure_new <- read.table("Data/mlvs_exposure.txt", 
                                  header=TRUE, sep='\t', check.names=TRUE, quote ="")
#z_mlvs_exposure_new <- merge(totom,z_mlvs_exposure_new,by="id")

z_idkey <- read.csv('Data/idkey.csv', header=TRUE, sep = ",", stringsAsFactors = FALSE)
...
z_plasma <- read.table("Data/plasma_fatty_acids_sjbae.txt",
                       header=TRUE, sep='\t', check.names=TRUE, quote ="")
...
write.table(z_mlvs_exposure_plasma, file = "Data/matching_exposure_plasma.tsv", sep = "\t", col.names = TRUE, row.names = FALSE)
