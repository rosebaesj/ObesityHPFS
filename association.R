...
association_bugs <- association_bugs %>%
  mutate(P.corpri = case_when(
    s__Prevotella_copri == 0 ~ 0,
    s__Prevotella_copri > 0 ~ 1))

association_bugs$P.corpri <- as.factor(association_bugs$P.corpri)
summary(association_bugs$P.corpri)
summary(association_bugs$s__Prevotella_copri)

summary(lmer(formula = bmi12 ~  calor_fs_dr + agemlvs+ abx+ (1 | participant), data=association_bugs))
summary(lmer(formula = bmi12 ~  P.corpri + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~  P.corpri + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~  P.corpri + calor_fs_dr +P.corpri*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~ P.corpri*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))

summary(lmer(formula = logcrp ~ bmi12+ calor_fs_dr + agemlvs+ abx+ (1 | participant), data=association_bugs))
summary(lmer(formula = logcrp ~ P.corpri+ calor_fs_dr + agemlvs+ abx+ bmi12+(1 | participant), data=association_bugs))



summary(glm(formula = loghdl ~ P.corpri* calor_fs_dr + agemlvs+ abx+ bmi12+(1 | participant), data=association_bugs))

#no significance



#### s__Lactobacillus_rogosae ###
# interaction_bug <- function(association_bugs, name = "species"){
#   species <- as.name(name)
#   association_bugs <- association_bugs %>%
#     mutate(grouped = case_when(
#       species == 0 ~ 0,
#       species > 0 ~ 1))
#   
#   association_bugs$grouped <- as.factor(association_bugs$grouped)
# 
#   return(summary(glm(formula = bmi12 ~ grouped*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs)))
#   
# }
# interaction_bug(association_bugs, name = "s__Prevotella_copri")
summary(association_bugs$s__Lactobacillus_rogosae)

association_bugs <- association_bugs %>%
  mutate(L.rogosae = case_when(
    s__Lactobacillus_rogosae == 0 ~ 0,
    s__Lactobacillus_rogosae > 0 ~ 1))

association_bugs$L.rogosae <- as.factor(association_bugs$L.rogosae)
summary(association_bugs$L.rogosae)


summary(lmer(formula = bmi12 ~  calor_fs_dr + agemlvs+ abx+ (1 | participant), data=association_bugs))

summary(glm(formula = bmi12 ~  s__Lactobacillus_rogosae + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~ s__Lactobacillus_rogosae*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))

summary(glm(formula = loghdl ~ s__Lactobacillus_rogosae*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))


###  s__Faecalibacterium_prausnitzii
summary(association_bugs$s__Faecalibacterium_prausnitzii)
...
summary(glm(formula = bmi12 ~  s__Faecalibacterium_prausnitzii + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~ s__Faecalibacterium_prausnitzii*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))


##### s__Bifidobacterium_adolescentis
summary(glm(formula = bmi12 ~  s__Bifidobacterium_adolescentis + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~ s__Bifidobacterium_adolescentis*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))

summary(association_bugs$s__Bifidobacterium_adolescentis)


association_bugs <- association_bugs %>%
  mutate(B.adolescentis = case_when(
    s__Bifidobacterium_adolescentis == 0 ~ 0,
    s__Bifidobacterium_adolescentis > 0 ~ 1))

association_bugs$B.adolescentis <- as.factor(association_bugs$B.adolescentis)
summary(association_bugs$B.adolescentis)

summary(lmer(formula = bmi12 ~  calor_fs_dr + agemlvs+ abx+ (1 | participant), data=association_bugs))
summary(lmer(formula = bmi12 ~  B.adolescentis + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~  B.adolescentis + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glmer(formula = bmi12 ~ B.adolescentis*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))

summary(lmer(formula = logcrp ~ bmi12+ calor_fs_dr + agemlvs+ abx+ (1 | participant), data=association_bugs))
summary(lmer(formula = logcrp ~ B.adolescentis+ calor_fs_dr + agemlvs+ abx+ bmi12+(1 | participant), data=association_bugs))
summary(lmer(formula = loghdl ~ B.adolescentis* calor_fs_dr + agemlvs+ abx+ bmi12+(1 | participant), data=association_bugs))

summary(lmer(formula = loghdl ~ B.adolescentis* calor_fs_dr + agemlvs+ abx +(1 | participant), data=association_bugs))
summary(lmer(formula = logtg ~ B.adolescentis* calor_fs_dr + agemlvs+ abx +(1 | participant), data=association_bugs))


summary(lmer(formula = s__Bifidobacterium_adolescentis ~ calor_fs_dr + agemlvs+ abx+ bmi12+(1 | participant), data=association_bugs))




##### s__Bifidobacterium_longum -> ns
summary(glm(formula = bmi12 ~  s__Bifidobacterium_longum + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~ s__Bifidobacterium_longum*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))

##### s__Bifidobacterium_pseudocatenulatum -> ns
summary(glm(formula = bmi12 ~  s__Bifidobacterium_pseudocatenulatum + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~ s__Bifidobacterium_pseudocatenulatum*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))

##### s__Bifidobacterium_bifidum -> ns
summary(glm(formula = bmi12 ~  s__Bifidobacterium_bifidum + calor_fs_dr + agemlvs + abx+(1 | participant), data=association_bugs))
summary(glm(formula = bmi12 ~ s__Bifidobacterium_bifidum*calor_fs_dr+ agemlvs + abx+(1 | participant), data=association_bugs))
...
ggplot(data=association_bugs, aes(bmi12, calor_fs_dr))+
  geom_point()

calor_fs_dr_w1avg	calor_fs_dr_w2avg
s__Lawsonibacter_asaccharolyticus
anova(lm.bmi)
summary(lm.bmi)

colnames(association_bugs)
