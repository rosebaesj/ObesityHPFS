...
# read in metadata (925)
mlvs_metadata_925 <- read.table('./maaslin2/pcl/mlvs_metadata_925.pcl', header=TRUE, sep='\t', check.names=FALSE, quote ="")
rownames(mlvs_metadata_925) <- mlvs_metadata_925 $sample
mlvs_metadata_925  <- mlvs_metadata_925 [,-1]
mlvs_metadata_925  <- as.data.frame(t(mlvs_metadata_925 ))
...
summary(mlvs_metadata_925$bmi12)


mlvs_metadata_925 <- mlvs_metadata_925 %>%
  mutate(bmi_f = case_when(
    0 < bmi12 & bmi12 <= 18.5 ~ 'Underweight',
    18.5 < bmi12 & bmi12<= 25 ~ 'Normal',
    25 <= bmi12 & bmi12< 30 ~ 'Overweight',
    30 <= bmi12 ~ 'Obese'))
mlvs_metadata_925$bmi_f <- factor(mlvs_metadata_925$bmi_f, 
                                     levels = c('Underweight', 'Normal', 'Overweight', 'Obese'))
summary(mlvs_metadata_925[stoolnum==1])
s1 <- mlvs_metadata_925 %>% filter(stoolnum==1)
summary(s1$bmi_f)
s2 <- mlvs_metadata_925 %>% filter(stoolnum==2)
summary(s2$bmi_f)
s3 <- mlvs_metadata_925 %>% filter(stoolnum==3)
summary(s3$bmi_f)
s4 <- mlvs_metadata_925 %>% filter(stoolnum==4)
summary(s4$bmi_f)
...
rownames(ord.scores) <- sub("^X", "", rownames(ord.scores))
# bug scores
ord.bug.scores <- as.data.frame(scores(bugs_pcoa, display = "species") )


# append metadata to scores
ord.scores.meta <- merge(ord.scores, mlvs_metadata_925, by = 'row.names')
...
library(viridis)
ordcolors <- scale_colour_gradientn(colours = viridis(10), limits=c(20,40))

histogram(s1$bmi12)

ggplot( ord.scores.meta, aes(MDS1, MDS2) ) + 
  geom_point(size=3, alpha = 0.75, aes( color = bmi_f)) + #change name
  #shape = factor(logcrpq) or factor(a_omega3_fs_der)
  #scale_shape_manual(values=c(3, 16, 17, 15), name = "CRP quartiles")+
  scale_color_manual(values=c("blue", "green", "yellow", "orange"), name = "by BMI")+
...
pcoa.plot <- 
  ggplot( ord.scores.meta, aes(MDS1, MDS2) ) + 
  geom_point(size=3, alpha = 0.75, aes( color = bmi12)) + #change name
  #shape = factor(logcrpq) or factor(a_omega3_fs_der)
  scale_shape_manual(values=c(3, 16, 17, 15), name = "CRP quartiles")+
  labs(color = "Short Term Trans Fat (g/day)")+ #change name
  theme_classic() + 
...
ggplot( ord.scores.meta, aes(MDS1, MDS2) ) + 
  geom_point(size=3, alpha = 0.75, aes( color = bmi_f, shape = bmi_f))+#, shape = logcrp_Q)) + #change name
  #shape = factor(logcrp_Q) +#or factor(a_omega3_fs_der)
  scale_shape_manual(values=c(16, 17, 15, 18, 1), name = "BMI")+
  labs(color = "BMI")+ #change name
  theme_classic() + 
  #theme_bw(base_size = 30) +
  theme(axis.line.x = element_line(colour = 'black', size=0.75, linetype='solid'),
        axis.line.y = element_line(colour = 'black', size=0.75, linetype='solid'),
